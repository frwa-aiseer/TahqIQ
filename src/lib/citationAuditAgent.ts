...
export async function runCitationAudit(input: CitationAuditInput, now = () => new Date().toISOString()): Promise<CitationAuditReport> {
  ...
  for (const { section, citationRef } of refs) {
    ...
    const integrityStatus = source.integrityVerification?.status;
    if (source.verificationState === "Retracted" || source.state === "Retracted" || source.retractionWarning || integrityStatus === "Retracted" || integrityStatus === "Expression of Concern") issues.push({ code: "RETRACTED_SOURCE", status: "WARNING", sourceId: source.id, citationRef, message: `Source '${source.id}' carries a retraction/expression-of-concern warning/state and requires researcher review.` });
    if (source.verificationState === "Corrected" || source.state === "Corrected" || source.correctionNotice || integrityStatus === "Corrected" || integrityStatus === "Updated") issues.push({ code: "CORRECTED_SOURCE", status: "WARNING", sourceId: source.id, citationRef, message: `Source '${source.id}' carries a correction/update notice: ${source.correctionNotice || source.integrityVerification?.message || "Researcher review required."}` });
